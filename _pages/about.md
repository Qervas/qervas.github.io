---
layout: about
title: about
permalink: /
subtitle: <a href='#'>Affiliations</a>. Address. Contacts. Moto. Etc.

profile:
  align: right
  image: me_snap.png
  image_circular: false # crops the image to make it circular
  address: 
    <p>Linköping, Sweden</p>

news: true  # includes a list of news items
latest_posts: true  # includes a list of the newest posts
selected_papers: true # includes a list of papers marked as "selected={true}"
social: true  # includes social icons at the bottom of the page
---
<h1 id="name" onmouseover="hoverName()" onmouseout="normalName()">Shaoxuan Yin</h1>
<div class="social-media-links">
  <a href="https://github.com/Qervas" target="_blank" rel="noopener noreferrer">
    <img src="../assets/img/icon/github.png" alt="GitHub" width="30" height="30">
  </a>
  <a href="https://www.linkedin.com/in/shaoxuan-yin-021548170/" target="_blank" rel="noopener noreferrer">
    <img src="../assets/img/icon/linkedin.png" alt="LinkedIn" width="30" height="30">
  </a>
  <a href="#" target="_blank" rel="noopener noreferrer">
    <img src="../assets/img/icon/youtube.png" alt="Twitter" width="30" height="30">
  </a>
<a href="#" target="_blank" rel="noopener noreferrer" style="position: relative; left: -9px;">
  <img src="../assets/img/icon/X.png" alt="X" width="45">
</a>



<script>
function hoverName() {
  document.getElementById("name").textContent = "Frank Yin";
}

function normalName() {
  document.getElementById("name").textContent = "Shaoxuan Yin";
}
</script>

<style>
#name {
  transition: all 0.3s ease;
  cursor: pointer;
}

#name:hover {
  color: #555; /* Change this to the color you want the text to change to on hover */
}
</style>

### Education

* **Linköping University, Sweden**
  * Master of Science in Computer Science (Sep 2023 - June 2025)
* **Beijing Information Science & Technology University, China**
  * Bachelor of Engineering in Computer Science and Technology (Sep 2019 - June 2023)
  * GPA: 3.5

### Skills Summary

* Proficient in  **Programming Languages** : C++, Python, C, Java, Mojo
* Skilled in  **Frameworks** : Qt, OpenGL, CUDA
* Strong command of English,  **IELTS** : 7.0
* Experienced with  **Tools & Technologies** : CMake, Git, Docker, Linux

### Internship Experience

* **GienTech Technology Co., Ltd.**
  * Software Testing Engineer (July 2021 - August 2021)
    * Worked on an online shopping mall applet project, writing scripts to test software for WeChat Mini Program.
* **Beijing Guoyao Xintiandi Information Technology Co., Ltd.**
  * C++ Development Engineer (August 2022 - September 2022)
    * Developed front-end code based on Qt5 and network protocol modules.

### Personal Projects

* **Robotics**
  * Led the design and development of a martial arts arena fighting wheeled robot, earning the third prize in the China Robotics Cup 2019.
* **[Computer Vision](../assets/video/STGCN.mp4)**
  * Conducted research on skiing action analysis using OpenPose and STGCN, resulting in a published paper.
* **[OpenGL Demos](https://github.com/Qervas/Playground/tree/main/Computer_Graphics/OpenGL)**
  * Crafted basic 3D components practices in OpenGL.
* **[Water Simulation](https://github.com/Qervas/water_simulation)**
  * Simulated water particles using SPH algorithm in OpenGL.
* **[Retro Vault (Unity 2D Game)](https://www.youtube.com/watch?v=cJxxEGaJsBo)**
  * Co-developed a 2D puzzle game, showcasing my knack for game design.

### Publications

* "Analysis of skiing action based on OpenPose." Intelligent Computer and Applications Vol.12 No.4 101-103.2022.

### Honors and Awards

* Secured the third prize in various national and regional contests, including the China Youth Cup Mathematical Contest, BISTU C Programming Contest, and BISTU Mathematics Competition.

In this space, I'll share my journey, insights, and creations. Join me as we delve into the intricacies of computer science, and perhaps, learn something new together.
